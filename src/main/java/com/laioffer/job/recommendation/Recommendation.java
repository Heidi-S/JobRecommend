...
public class Recommendation {
    ...
    public List<Item> recommendItems(String userId, double lat, double lon){
        ...
        //Step 2 get all key words,sort by count
        Map<String, Integer> allKeywords = new HashMap<>();
        for (String itemId : favoritedItemsIds){
        ...
        //Step 3 search based on keywords, filter out favorited items
        ...
        for (Map.Entry<String, Integer> keyword : keywordList){
            ...
            for (Item item : items){
                if (!favoritedItemsIds.contains(item.getId()) && !visitedItemIds.contains(item.getId())){
